...
int main (int argc, char **argv)
{
    ...
    // return 0;
    const char* imageName = "red-green-16x16.png";
    char fullPath[256];
    snprintf(fullPath, sizeof(fullPath), "./starter/images/%s", imageName);
    ...
    char *path = ".";
    char** png_files = findpng(path);
    if (!png_files) {
        fprintf(stderr, "No PNG files found or error occurred.\n");
        return 1;
    }

    printf("Found PNG files in \"%s\":\n", path);
